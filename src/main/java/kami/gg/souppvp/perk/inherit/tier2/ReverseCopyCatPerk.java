    ...
    public void onPlayerDeathEvent(PlayerDeathEvent event){
        ...
        if (currentPerk == reverseCopyCatPerk){
            if (new Random().nextInt(100) >= 50){
                event.getEntity().getKiller().playSound(event.getEntity().getKiller().getLocation(), Sound.ENTITY_CAT_AMBIENT, 1F, 1F);
                ...
                Bukkit.getScheduler().scheduleSyncDelayedTask(SoupPvP.getInstance(), () ->
                        event.getEntity().playSound(event.getEntity().getLocation(), Sound.ENTITY_CAT_HISS, 1F, 1F), 40L);
            }
        }
    }
